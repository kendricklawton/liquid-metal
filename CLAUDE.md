...
- **Predictable specs** — developers declare vCPU/RAM in `liquid-metal.toml`; no mystery resource pools
...
└── cmd/                package cmd — Cobra commands, importable for tests
    ...
    └── deploy.go       flux deploy (reads liquid-metal.toml → CreateService)
```
