...
pub async fn run(config: &mut Config, invite: Option<String>) -> Result<()> {
    ...
    let da = device_authorize(&http, &cli_cfg.device_auth_url, &cli_cfg.client_id).await?;
    ...
    let info = fetch_user_info(&http, &cli_cfg.userinfo_url, &tokens.access_token).await?;
    ...
    config.oidc_sub = Some(pr.oidc_sub);
...
async fn device_authorize(http: &Client, device_auth_url: &str, client_id: &str) -> Result<DeviceAuthResponse> {
    let resp = http
...
async fn fetch_user_info(http: &Client, userinfo_url: &str, access_token: &str) -> Result<UserInfo> {
