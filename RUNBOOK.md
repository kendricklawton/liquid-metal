...
```bash
# Arch Linux
sudo pacman -S rustup go-task   # go-task is optional
...
> `~/.cargo/bin` must be on your `PATH`. Rustup adds this automatically; if not, add `export PATH="$PATH:$HOME/.cargo/bin"` to your shell rc file (`~/.bashrc`, `~/.zshrc`, etc.).
...
| `GCP_KMS_KEY`          | Yes      | Full Cloud KMS cryptoKey resource name (see [First-Time Setup](#first-time-setup)) |
...
| `COOKIE_SECRET`        | No       | Optional for dev — auto-generated with warning. Set for persistent sessions        |

> **First-time setup?** See [First-Time Setup](#first-time-setup) below for Zitadel OIDC app creation and GCP KMS key ring provisioning.

Everything else has safe defaults. See `.env.example` for the full list with documentation.
...
Open 4 terminal tabs (5 if running the proxy).
...
docker compose up -d
```
Starts: Postgres (:5432), NATS (:4222), MinIO S3 mock (:9000, console :9001)
...
```
Waits for: `Listening on 0.0.0.0:7070` and `GCP KMS client initialized`
...
> On non-Linux platforms: Firecracker/TAP are skipped automatically. Only Liquid (Wasm) deployments work locally.
...
# Without Taskfile
RUST_LOG="web=debug" WEB_BIND_ADDR="0.0.0.0:3000" cargo run -p web
```
Waits for: `web listening bind=0.0.0.0:3000`

Browse to `http://localhost:3000` to see the marketing splash page. The API (Tab 2) must be running for authenticated dashboard routes to work.

> `COOKIE_SECRET` is optional in dev — an ephemeral key is generated automatically with a warning. Set it in `.env` to keep sessions alive across web restarts.

### Tab 5 — Proxy (optional)

Only needed if you want to test slug-based routing (e.g. `markdown-renderer.localhost:8080`).
...
# Without Taskfile
BIND_ADDR="0.0.0.0:8080" CLOUD_UPSTREAM="127.0.0.1:7070" RUST_LOG="proxy=debug" cargo run -p proxy
```
Listens on `:8080`. Routes `{slug}.localhost:8080` → the service's `upstream_addr` via DB lookup.

---

## Authenticate

The CLI defaults to `https://api.liquidmetal.dev`. For local dev, point it at your local API:
...
# First-time signup with invite code
flux login --invite <code>
```
...
flux deploy    # builds main.wasm → uploads to MinIO → API → NATS → daemon runs it
```
...
```bash
# Replace PORT with the value from flux services UPSTREAM column
curl http://127.0.0.1:PORT/

# Or open in browser
flux open <service-slug>
```
...
flux logs <service-slug>
flux logs <service-slug> --limit 500
flux logs <service-slug> --follow   # live tail (polls every 2s)
```
...
Common commands after your first deploy:
...
flux env set <service> KEY=VALUE  # set an env var (encrypted at rest via KMS)
flux logs <service> --follow      # live tail logs
```

> Environment variables are encrypted at rest using envelope encryption (AES-256-GCM + GCP Cloud KMS). The API encrypts on write and decrypts on read — no plaintext env vars are stored in the database.

Run `flux --help` for the full command reference (domains, billing, workspaces, rollbacks, etc.).
...
When the API is running, interactive API docs are available:
...
All endpoints are documented with request/response schemas, query parameters, and security requirements. Use the **Authorize** button in Swagger UI to set `X-Api-Key` (user token) or `X-Internal-Secret` (internal calls) for authenticated requests.
...
| Symptom                                    | Check                                                                                                |
|--------------------------------------------|------------------------------------------------------------------------------------------------------|
| `flux: command not found`                  | Add `$HOME/.cargo/bin` to `PATH` and reinstall                                                       |
| `flux login` fails                        | `OIDC_CLI_CLIENT_ID` in `.env`, confirm Docker infra is running. Check `OIDC_ISSUER` is reachable   |
| `error: fetch CLI config`                 | CLI can't reach the API — set `export API_URL=http://localhost:7070` for local dev                   |
| `unauthorized_client` on `flux login`     | Zitadel CLI app missing `device_code` grant type — re-enable in Zitadel console                      |
| API fails: `GCP_KMS_KEY is required`      | Set `GCP_KMS_KEY` in `.env` — see [First-Time Setup](#first-time-setup)                              |
| API fails: `initializing GCP KMS client`  | Check `GCP_KMS_CREDENTIALS` points to a valid service account JSON with KMS permissions              |
| API fails: `CERT_DEK_WRAPPED not set`     | Generate it — see [Generate `CERT_DEK_WRAPPED`](#generate-cert_dek_wrapped)                          |
| API: `ACME account created` warning       | Copy the JSON from logs into `.env` as `ACME_ACCOUNT_KEY` to persist across restarts                 |
| `flux init` fails                         | API running? (`cargo run -p api` or `task dev:api`)                                                  |
| `flux deploy` upload fails                | MinIO running? (`docker compose up -d minio`) Check `http://localhost:9001`                          |
| Status stuck at `provisioning`            | Check daemon logs for errors                                                                         |
| `GetUploadUrl` error                      | API can't reach MinIO — check `.env` S3 vars                                                         |
| Web won't start: `INTERNAL_SECRET not set`| Add `INTERNAL_SECRET=change-me-in-production` to `.env`                                              |
| Web login redirects to wrong URL          | Set `WEB_PUBLIC_URL=http://localhost:3000` in `.env`                                                 |
| Web sessions lost on restart              | Set `COOKIE_SECRET=$(openssl rand -hex 32)` in `.env`                                                |
| Web can't reach API                       | Confirm Tab 2 is running; check `API_URL` in `.env` (default `http://localhost:7070`)                |
| OIDC discovery fails at web startup       | Check `OIDC_ISSUER` is correct and reachable; web uses `authorization_endpoint` (browser flow)       |
